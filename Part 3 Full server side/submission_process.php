...
if (isset($_POST['submitPost'])){// if input is posted
	...
	//$target_file = $target_dir . $rename.basename($_FILES["myImage"]["name"]); for xampp local 
	...
	if(count($errors)>0){// if error exist
		...
		//move_uploaded_file($_FILES["myImage"]["tmp_name"], $target_file); xampp local 
			//aws s3 upload
		$awsAccessKey ="AKIAI7V3G7D7Y4IK3EPA";
		$awsSecretKey ="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";
		$bucketname ="jimhuuploadfiles";
		require("S3.php");// include the download S3 php with function to upload file to S3 bucket
		$s3 = new S3($awsAccessKey, $awsSecretKey);
		$ok = $s3->putObjectFile($_FILES["myImage"]["tmp_name"], $bucketname, $target_file, S3::ACL_PUBLIC_READ);
		}
		...
		$url ='https://s3.amazonaws.com/'.$bucketname.'/'.$target_file;// create S3 url
	}
